...
namespace PrismLog.Editor.Window
{
    internal class LogLine : VisualElement
    {
        ...
        private PrismLogSettings _settings = null;
        ...
        public void SetSearchResult(int startIndex, int endIndex)
        {
            ...
            Message = string.Format("{0}<color={1}>{2}</color>{3}", startmessage, PrismConsoleWindowUtility.HighlightBackgroundColor, midmessage, endmessage);
        }
        ...
        public LogLine()
        {
            _settings = PrismLogSettings.GetOrCreateSettings();
