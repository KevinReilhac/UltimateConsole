...
namespace PrismLog
{
    public class LogFilters
    {
        ...
        public LogFiltersResult CheckLog(PLog log)
        {
        ...
        private bool CheckLogType(PLog log)
        {
            return _logTypes.Count > 0 && !_logTypes.Contains(log.logType);
        }

        private LogFiltersResult CheckLogForException(PLog log)
        {
            LogFiltersResult result = new LogFiltersResult(true);
            ...
            if (Settings.ExceptionDisplayMode.HasFlag(PrismLogSettings.EExceptionDisplayMode.CheckLogType))
            {
